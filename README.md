...
Distance/travel time between points (via a mapping API such as OpenStreetMap or Google Maps Distance Matrix API)
...
Take a small list of delivery addresses (5–10 stops) as input
Build a distance matrix, similar to the pineapple shipping exercise in this course
Use simulated annealing to find a low-cost route
Output the suggested route order and estimated total distance/time
Challenges
...
Acknowledgments
Route optimization concepts (state space, transitions, brute force, hill climbing, simulated annealing) as taught in the Building AI and Elements of AI courses by Reaktor and the University of Helsinki
